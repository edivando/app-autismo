...
public class RFAboutViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,MFMailComposeViewControllerDelegate,SFSafariViewControllerDelegate {
    ...
    public var closeButtonImage: UIImage? = UIImage(named: "RFAboutViewCloseX")
    ...
    public override func loadView() {
        ...
        tableHeaderLabel.text = NSLocalizedString(String(format: "%@ makes use of the following third party libraries. Many thanks to the developers making them available!".localized,self.appName!) as String, comment: "Acknowlegdments header title")
    ...
    public func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if let info = theDict?.objectForKey("content") as? String {
            if info.containsString("http://") || info.containsString("https://") {
                goToWebsite(NSURL(string: info)!)
            } else {
    ...
    public func goToWebsite(url: NSURL) {
        if #available(iOS 9.0, *) {
